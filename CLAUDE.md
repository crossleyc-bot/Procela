...
>
> **Status (shipped):** the Discover loop is now built for direct-connect sources — real catalog discovery across relational databases (Postgres/MySQL/SQL Server/Oracle/Redshift), cloud warehouses (Snowflake/BigQuery/Databricks), MongoDB, and cloud object storage (S3/Azure Blob/GCS/SFTP, incl. Parquet/Avro), plus local-file uploads; measured data quality run live over direct database connections; and a suggest-and-confirm flow that reconciles discovered assets into the governed catalog. The on-prem edge connector covers firewalled relational sources. Only API/spreadsheet sources remain simulated. The remaining Phase-3 item is an operational pilot against a real customer database (see `docs/STATUS.md` — Track A, and Track E / "Discovery coverage" for the non-relational build-out).

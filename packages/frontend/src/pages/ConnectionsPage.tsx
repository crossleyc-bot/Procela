      ...
      case 'DATA_WAREHOUSE':
        return (
          <>
            ...
            {fieldRow(form.config.warehouseType === 'BIGQUERY' ? 'Project ID' : form.config.warehouseType === 'DATABRICKS' ? 'Server Hostname' : 'Account', <input aria-label="Account" style={inputStyle} value={form.config.account || ''} onChange={(e) => updateConfig('account', e.target.value)} placeholder={form.config.warehouseType === 'REDSHIFT' ? 'cluster endpoint host' : form.config.warehouseType === 'BIGQUERY' ? 'e.g. my-gcp-project' : form.config.warehouseType === 'DATABRICKS' ? 'e.g. dbc-xxxx.cloud.databricks.com' : 'e.g. org-account'} />)}
            {form.config.warehouseType !== 'BIGQUERY' && fieldRow(form.config.warehouseType === 'DATABRICKS' ? 'HTTP Path' : 'Warehouse', <input aria-label="Warehouse" style={inputStyle} value={form.config.warehouse || ''} onChange={(e) => updateConfig('warehouse', e.target.value)} placeholder={form.config.warehouseType === 'DATABRICKS' ? '/sql/1.0/warehouses/xxxx' : 'e.g. COMPUTE_WH'} />)}
            {(form.config.warehouseType === 'SNOWFLAKE' || form.config.warehouseType === 'BIGQUERY' || form.config.warehouseType === 'DATABRICKS') && fieldRow(form.config.warehouseType === 'BIGQUERY' ? 'Dataset' : form.config.warehouseType === 'DATABRICKS' ? 'Catalog' : 'Database', <input aria-label="Database" style={inputStyle} value={form.config.database || ''} onChange={(e) => updateConfig('database', e.target.value)} placeholder={form.config.warehouseType === 'BIGQUERY' ? 'e.g. analytics' : form.config.warehouseType === 'DATABRICKS' ? 'e.g. main' : 'e.g. ANALYTICS'} />)}
            {(form.config.warehouseType === 'SNOWFLAKE' || form.config.warehouseType === 'DATABRICKS') && fieldRow('Schema', <input aria-label="Schema" style={inputStyle} value={form.config.schema || ''} onChange={(e) => updateConfig('schema', e.target.value)} placeholder={form.config.warehouseType === 'DATABRICKS' ? 'optional — defaults to default' : 'optional — defaults to PUBLIC'} />)}

            {/* BigQuery: service-account JSON (or blank for ADC) */}
            {form.config.warehouseType === 'BIGQUERY' && fieldRow('Service Account JSON', <input aria-label="Service Account JSON" style={inputStyle} type="password" value={form.credentials.token || ''} onChange={(e) => updateCreds('token', e.target.value)} placeholder={editingId ? '(unchanged if left blank)' : 'blank = Application Default Credentials'} />)}

            {/* Databricks: personal access token */}
            {form.config.warehouseType === 'DATABRICKS' && fieldRow('Access Token', <input aria-label="Access Token" style={inputStyle} type="password" value={form.credentials.token || ''} onChange={(e) => updateCreds('token', e.target.value)} placeholder={editingId ? '(unchanged if left blank)' : 'Databricks personal access token'} />)}

            {/* User/pass — not used by BigQuery (key/ADC auth) or Databricks (token auth) */}
            {form.config.warehouseType !== 'BIGQUERY' && form.config.warehouseType !== 'DATABRICKS' && fieldRow('Username', <input aria-label="Username" style={inputStyle} value={form.credentials.username || ''} onChange={(e) => updateCreds('username', e.target.value)} placeholder="Username" />)}
            {form.config.warehouseType !== 'BIGQUERY' && form.config.warehouseType !== 'DATABRICKS' && fieldRow('Password', <input aria-label="Password" style={inputStyle} type="password" value={form.credentials.password || ''} onChange={(e) => updateCreds('password', e.target.value)} placeholder={editingId ? '(unchanged if left blank)' : 'Password'} />)}
          </>
        );
